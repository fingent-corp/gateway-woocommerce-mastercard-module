 ...
 *
 */
const settings = window.wc.wcSettings.getSetting( 'mastercard_gateway_data', {} ),
    label = window.wp.htmlEntities.decodeEntities( settings.title ) || window.wp.i18n.__( 'Mastercard Gateway', 'mastercard_gateway' ),
    Content = () => {
        return window.wp.htmlEntities.decodeEntities( settings.description || '' );
    },
    Mastercard_Block_Gateway = {
        name: 'mastercard_gateway',
        label: label,
...
jQuery( function( $ ) {
    if ( ( 'undefined' !== typeof wc &&  wc?.blocksCheckout ) && ( typeof handlingText !== 'undefined' && handlingText ) ) {
        ...
        function selectPaymentMethod() { 
            const selectedPaymentMethod = wp.data.select( 'wc/store/payment' ).getActivePaymentMethod() || null; 
            if( 'mastercard_gateway' === selectedPaymentMethod ) {
                if( $( element ).length < 0 ) {
