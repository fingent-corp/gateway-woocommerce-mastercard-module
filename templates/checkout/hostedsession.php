...
<form class="mpgs_hostedsession wc-payment-form" action="<?php echo esc_url( $gateway->get_payment_return_url( $order->get_id() ) ); ?>" method="post">
	...
	<p class="form-row form-row-wide">
		<button type="button" id="mpgs_pay" onclick="mpgsPayWithSelectedInstrument()"><?php esc_html_e( 'Pay', 'mastercard' ); ?></button>
	</p>
</form>
...
<script type="text/javascript">
	...
	function hsErrorsMap() {
		return {
			cardNumber: "<?php esc_html_e( 'Invalid Card Number', 'woocommerce' ); ?>",
			securityCode: "<?php esc_html_e( 'Invalid Security Code', 'woocommerce' ); ?>",
			expiryMonth: "<?php esc_html_e( 'Invalid Expiry Month', 'woocommerce' ); ?>",
			expiryYear: "<?php esc_html_e( 'Invalid Expiry Year', 'woocommerce' ); ?>"
		};
	}
	...
	(function ($) {
		...
		$.when( createSession() ).done( function ( response ) {
			if ( is3DsV2Enabled() ) {
				ThreeDS.configure({
					merchantId: '<?php echo esc_attr( $gateway->get_merchant_id() ); ?>',
					sessionId: response.session.id,
					containerId: "3DSUI",
					callback: function () {
					},
					configuration: {
						wsVersion: <?php echo esc_attr( $gateway->get_api_version_num() ); ?>
					}
				});
			}
		...
		function initiateAuthentication() {
			...
			ThreeDS.initiateAuthentication(
				'<?php echo esc_attr( $gateway->add_order_prefix( $order->get_id() ) ); ?>',
				txnId,
				function ( data ) {
		...
		function savePayment( data ) {
			return $.ajax({
				url: '<?php echo esc_attr( $gateway->get_save_payment_url( $order->get_id() ) ); ?>',
				method: 'post',
				data: data,
				dataType: 'json'
			});
		...
		function createSession() {
			return $.ajax({
				url: '<?php echo esc_attr( $gateway->get_create_session_url( $order->get_id() ) ); ?>',
				method: 'get',
				dataType: 'json'
			});
		}
